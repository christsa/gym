...
def reward_function_eval(achieved_goal, goal, idx_goals, interaction, robot0_joint_names):
    """
    ...
    # Force error eval for handclap is different than the rest
    if interaction == 'handclap':
        if np.sum(achieved_goal[2:4] > 0) and np.sum(achieved_goal[4:6] > 0) and np.sum(
                achieved_goal[6:8] > 0) and np.sum(achieved_goal[8:10] > 0):
            force_err = 0
        else:
            force_err = 1
    else:
        if np.sum(goal[:idx_goals[1]] - achieved_goal[:idx_goals[1]], axis=-1) < 1:
            force_err = 0
        else:
            force_err = 1

    return np.sum(np.linalg.norm(dist_reshaped, axis=-1), axis=-1), force_err, fc_angle_error / len(robot0_joint_names), \
           imi_angle_error / len(robot0_joint_names), np.sum(np.linalg.norm(imi_pos_error, axis=-1), axis=-1)


def reward_function(achieved_goal, goal, w_p, idx_goals, reward_weights):
    """
    ...
    try:
        # Force error
        force_err = np.sum(np.abs(goal[:, :idx_goals[1]] - achieved_goal[:, :idx_goals[1]]), axis=-1)
        # Final state position error
        dist_reshaped = np.reshape(achieved_goal[:, idx_goals[1]:idx_goals[2]] - goal[:, idx_goals[1]:idx_goals[2]],
                                   (-1, 5, 3))
        # Final state angle error
        fc_angle_error = np.sum(
            np.abs(achieved_goal[:, idx_goals[2]:idx_goals[3]] - goal[:, idx_goals[2]:idx_goals[3]]), axis=-1)
        # Imitation state angle error
        imi_angle_error = np.sum(
            np.abs(achieved_goal[:, idx_goals[3]:idx_goals[4]] - goal[:, idx_goals[3]:idx_goals[4]]), axis=-1)
        # Imitation state position error
        imi_pos_error = np.reshape(achieved_goal[:, idx_goals[-1]:] - goal[:, idx_goals[-1]:], (-1, 5, 3))
        ...
        dist_reshaped = np.reshape(achieved_goal[idx_goals[1]:idx_goals[2]] - goal[idx_goals[1]:idx_goals[2]],
                                   (-1, 5, 3))
        fc_angle_error = np.sum(np.abs(achieved_goal[idx_goals[2]:idx_goals[3]] - goal[idx_goals[2]:idx_goals[3]]),
                                axis=-1)
        imi_angle_error = np.sum(np.abs(achieved_goal[idx_goals[3]:idx_goals[4]] - goal[idx_goals[3]:idx_goals[4]]),
                                 axis=-1)
        imi_pos_error = np.reshape(achieved_goal[idx_goals[-1]:] - goal[idx_goals[-1]:],
                                   (-1, 5, 3))
...
class HRIEnv(robot_env.RobotEnv):
    ...
    def _get_achieved_goal(self):
        ...
        goal = np.hstack((gripper_array, np.array(robot_pos).flatten()))
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        """
        cost = reward_function(achieved_goal, goal, self.weights, self.idx_goals, self.reward_weights)
        return cost
    ...
    def _get_obs(self):
        ...
        obs = np.concatenate([
            joint_qpos, joint_vels,
            np.array(body_pos).flatten(), np.array(gripper_force).flatten()
        ])
    ...
    def _is_success(self, achieved_goal, desired_goal):
        ...
        pos_error, force_error, fc_angle_error, imi_angle_error,\
            imi_pos_error = reward_function_eval(achieved_goal, desired_goal, self.idx_goals, self.interaction, self.robot0_joint_names)
